...
def draw_results(image: numpy.ndarray, report: dict[str, Any]) -> numpy.ndarray:
    """Render face detection bboxes and 5-point landmarks onto a resized canvas.
    ...
    * Face N (N >= 1) — magenta dashed rectangle.
    ...
    Args:
        ...
        report: Algorithm result dict with per-face lists:
            ``face_detection`` (``xyxy``: list[4 float], ``score``: float),
            ``face_landmarks`` (``landmarks``: list[10 float]).
    ...
    for _face_idx, lm in enumerate(face_landmarks):
        ...
        if len(landmarks) >= 10:
            ...
            for px, py in pts:
                cv2.circle(vis, (px, py), 3, COLOR_LANDMARK, -1, lineType=LINE_STYLE_SOLID)
...
def main():
    ...
    out_path = f"{stem}_retinaface_result{ext}"
    cv2.imwrite(out_path, vis)
    print(f"Saved result to: {out_path}")

    cv2.imshow("retinaface", vis)
    cv2.waitKey(0)
