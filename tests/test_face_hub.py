...
import seetapsych_face_hub as pkg
...
def _collect_module_ymls() -> list[Path]:
    modules_dir = Path(os.path.dirname(__file__)).parent / "seetapsych_face_hub" / "modules"
...
_MODULES_DIR = Path(os.path.dirname(__file__)).parent / "seetapsych_face_hub" / "modules"


def _yml_id(p: Path) -> str:
    return str(p.relative_to(_MODULES_DIR)).replace("\\", "/")


@pytest.mark.parametrize("yml_path", _MODULE_YMLS, ids=_yml_id)
